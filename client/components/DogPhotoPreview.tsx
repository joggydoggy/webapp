...
export default function DogPhotoPreview({ dog, design }: DogPhotoPreviewProps) {
  ...
  const detectDogShape = async () => {
    ...
    try {
      ...
      if (measurementPoints.length === 0) {
        ...
        const autoPoints = generateAutoMeasurementPoints(
          imageRect.width,
          imageRect.height,
        );
        setMeasurementPoints(autoPoints);
        localStorage.setItem(
          "dogMeasurementPoints",
          JSON.stringify(autoPoints),
        );
      }
    } catch (error) {
  ...
  const generateAutoMeasurementPoints = (
    width: number,
    height: number,
  ): MeasurementPoint[] => {
    return [
    ...
    return (
      <div className="bg-muted/50 rounded-lg p-8 text-center">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Upload a photo in the Measurements section to see your dog wearing the
          custom design.
        </p>
      ...
      >
        <defs>
          ...
          >
            <stop
              offset="0%"
              stopColor={design.primaryColor}
              stopOpacity="0.9"
            />
            <stop
              offset="50%"
              stopColor={design.secondaryColor}
              stopOpacity="0.8"
            />
            <stop
              offset="100%"
        ...
        {design.style === "sporty" && measurementPoints.length > 0 && (
          <>
            ...
            <line
              ...
              x2={
                (measurementPoints.find((p) => p.id === "back-end")?.x || 0) *
                0.7
              }
            ...
            <line
              x1={
                measurementPoints.find((p) => p.id === "chest-bottom")?.x || 0
              }
              y1={
                (measurementPoints.find((p) => p.id === "chest-bottom")?.y ||
                  0) - 20
              }
              x2={
                (measurementPoints.find((p) => p.id === "back-end")?.x || 0) *
        ...
        {design.style === "modern" && measurementPoints.length > 0 && (
          <>
            ...
            <circle
              cx={
                (measurementPoints.find((p) => p.id === "chest-top")?.x || 0) +
                20
              }
              cy={
                (measurementPoints.find((p) => p.id === "chest-top")?.y || 0) +
                10
              }
              r="8"
              fill="white"
